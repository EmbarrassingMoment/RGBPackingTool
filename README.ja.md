...
- **4チャンネルパッキング (RGBA)**: 最大4枚の入力テクスチャを受け取り、それぞれ選択したチャンネル（デフォルトは Red）を出力テクスチャの R、G、B、A チャンネルに割り当てます。
...
- **アンパックモード (逆パッキング)**: ツールウィンドウ上部の **パック / アンパック** 切り替えで2つのワークフローを行き来できます。アンパックタブは、パック済みの RGBA テクスチャを個別のグレースケールアセットに分解します:
  - **即時チャンネルプレビュー**: ソーステクスチャを選択すると、R/G/B/A の内容が 2×2 のグレースケールプレビューグリッドとして即座に表示されます。
  - **チャンネル別出力トグル**: どのチャンネルをアセットとして出力するかを個別に選択できます。
  - **未使用チャンネル検出**: 全ピクセルが同じ値のチャンネル（例: 全面 255 のアルファ）には「均一」バッジが表示され、自動的に出力対象から除外されます（出力したい場合はチェックを入れ直せます）。
  - **スマート命名**: ベース名はソース名から既知のパックサフィックスを除去して生成され（`T_Rock_ORM` → `T_Rock`）、各チャンネルにはプリセット由来のサフィックスが付加されます（ORM の場合: `_AO`、`_Roughness`、`_Metallic`）。
  - **プリセット連動サフィックス**: プリセットがチャンネルごとのアンパックサフィックスを保持します。既存のプリセットファイルは `_R`/`_G`/`_B`/`_A` をデフォルトとして読み込まれます。
  - **省メモリ設計**: プレビュー・抽出ともにフル解像度の中間バッファを作らずソースから直接読み出すため、16K テクスチャのプレビューでもギガバイト単位ではなく数百KB程度で済みます。
...
- **Unreal Engine 5.5以上** (5.8で開発およびテスト済み)
...
1. **ツールの起動**:
   Unreal Editor のメインメニューバーから **Tools (ツール) > Texture Channel Packer** を選択します。Texture Channel Packer タブが開きます。このタブはエディタレイアウトの任意の場所にドッキングできます。

2. **プリセットの選択**（任意）:
   ツール上部の **プリセット** ドロップダウンから、ワークフローに合わせた構成を素早く設定できます：
   - **ORM**（デフォルト）: R=アンビエントオクルージョン、G=ラフネス、B=メタリック — サフィックス `_ORM`
   - **MRA**: R=メタリック、G=ラフネス、B=アンビエントオクルージョン — サフィックス `_MRA`
   - **Custom**: プリセットから手動で設定を変更すると自動的に表示されます。

   さらに以下の操作が可能です：
   - **名前を付けて保存...**: 現在の設定（チャンネルラベル、圧縮設定、反転フラグ、入力チャンネル選択、ファイル名サフィックス、チャンネルごとのアンパックサフィックス）を名前付きプリセットとして保存します。
   - **削除**: ユーザー作成のプリセットを削除します（組み込みプリセットは削除できません）。

   ユーザープリセットは `<プロジェクトディレクトリ>/Saved/TextureChannelPacker/Presets/` に JSON ファイルとして保存されます。チームでプリセットを共有するには、これらのファイルを他のメンバーのプロジェクトにコピーするだけです。
...
**アンパック** タブはパッキングの逆の処理を行い、パック済みテクスチャの R/G/B/A チャンネルを個別のグレースケールアセットとして抽出します。

1. **アンパックに切り替え**:
   ツールウィンドウ上部の切り替えで **アンパック** をクリックします。

2. **プリセットの選択**（任意）:
   プリセットはチャンネルごとの出力サフィックスを決定します（例: ORM の場合 `_AO` / `_Roughness` / `_Metallic`）。プリセットの管理はパックタブで行い、アンパックタブの選択はパックタブとは独立しています。

3. **ソーステクスチャの選択**:
   分解するパック済みテクスチャを選択します。4つのチャンネルの内容が 2×2 のグレースケールグリッドとして即座にプレビュー表示されます。
   - **均一** と判定されたチャンネル（全ピクセルが同じ値、例: 未使用のアルファ 255）にはバッジが表示され、自動的にチェックが外れます。

4. **チャンネルと出力の設定**:
   - 各チャンネルセルのチェックボックスで、出力するチャンネルを選択します。
   - **ベースファイル名** はソース名から既知のパックサフィックスを除去して自動生成され（`T_Rock_ORM` → `T_Rock`）、各セルには最終的なアセット名（例: `T_Rock_AO`）が表示されます。ベース名は手動でも編集できます。
   - **Output Path** は直接入力するか、フォルダアイコンから指定します。

5. **アンパック実行**:
   **テクスチャをアンパック** をクリックします。選択した各チャンネルにつき1枚のグレースケールアセット（G8、圧縮設定 `Grayscale`、`sRGB = false`）がソースと同じ解像度で作成されます。既存アセットがある場合は、書き込み前に対象を一覧表示する上書き確認ダイアログが表示されます。
...
**Q: プリセットをチームで共有するにはどうすればよいですか？**
**A:** ユーザー作成のプリセットは `<プロジェクトディレクトリ>/Saved/TextureChannelPacker/Presets/` に個別の JSON ファイルとして保存されます。これらの `.json` ファイルを他のメンバーのプロジェクトの同じディレクトリにコピーしてください。次回エディタ起動時にプリセットドロップダウンに表示されます。

**Q: プリセットにはどのような設定が保存されますか？**
**A:** プリセットには、チャンネルラベルの説明（英語・日本語）、自動ファイル名のサフィックス（例: `_ORM`）、デフォルトの圧縮設定、各チャンネルのデフォルト反転フラグ、デフォルトの入力チャンネル選択、チャンネルごとのアンパックサフィックス（例: `_AO`）が保存されます。入力テクスチャや出力パスはプリセットに**含まれません**。

**Q: パック済みテクスチャを元の個別テクスチャに分解できますか？**
**A:** はい。**アンパック** タブに切り替えてパック済みテクスチャを選択し、**テクスチャをアンパック** をクリックしてください。選択した各チャンネルが個別のグレースケール（G8）アセットとして保存されます。未使用と思われるチャンネル（均一値）は自動的に除外され、出力名はプリセットのチャンネルサフィックスに従います。
