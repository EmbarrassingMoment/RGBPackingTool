...
- **結果プレビュー (Live Preview)**: アセットを生成する前に、パック後の RGB 合成結果を低解像度で表示するプレビューパネルを搭載しています。**プレビュー更新** ボタンを押すと、現在の入力・反転フラグ・入力チャンネル選択が反映されます。プレビューは小さな解像度に制限されるため 16K ターゲットでも高速で、生成アセットと同じリニア (sRGB オフ) で表示されます。
- **ユーザーインターフェース**:
...
**A:** プリセットには、チャンネルラベルの説明（英語・日本語）、自動ファイル名のサフィックス（例: `_ORM`）、デフォルトの圧縮設定、各チャンネルのデフォルト反転フラグが保存されます。入力テクスチャや出力パスはプリセットに**含まれません**。
