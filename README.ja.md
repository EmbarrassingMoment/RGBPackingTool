...
- **入力処理**:
  ...
  - **チャンネル反転機能**: 各チャンネルごとに「反転 (Invert)」チェックボックスがあります。これを使用すると、パッキング時に特定のチャンネルを反転（例: ラフネスをグロシネスに変換、ノーマルマップのチャンネル反転など）させることができます。
- **拡張フォーマットサポート**:
