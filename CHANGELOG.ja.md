...
### 追加 (Added)
- **設定の永続化 (Settings Persistence)**: 最後に使用した設定をエディタ再起動後も記憶するようになりました。出力パス、解像度（幅・高さ）、選択中のプリセット、圧縮設定、反転フラグ、入力チャンネル選択が、変更のたびに `EditorPerProjectUserSettings.ini` へ保存され、次回エディタ起動時に復元されます。
  - 保存されていたプリセットが削除済みのユーザープリセットだった場合は、デフォルト（ORM）プリセットにフォールバックします。
  - 復元した個別の設定値がプリセットのデフォルトと一致しない場合（例: 前回セッションが「Custom」状態で終了した場合）は、実際の編集操作と同様にプリセット表示が「Custom」になります。
  - 入力テクスチャと出力ファイル名は、パッキング作業ごとに異なるため意図的に保存対象外としています。
